# Naive Baseline For Causality Extraction Task 2 @ Touché 2026

This baseline implementation always predicts the first and the second half of a text to be entities. You can use it as a starting point for developing your TIRA submission. Simply edit `predict.py` to assign `df['entity']` entity spans based on your approach.
...
```
tira-cli code-submission --path . --task causality-extraction-toucheclef26 --dataset task2-spot-check-dataset-20260430-training --dry-run --command '/predict.py --dataset $inputDataset --output $outputDir'
```