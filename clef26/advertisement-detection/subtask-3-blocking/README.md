# Baseline for Touché'26 Advertisement Blocking

This directory contains a baseline for blocking advertisements in a RAG response by removing them. 
This baseline takes the RAG response and character offsets ('spans' of advertising text) as input.
...
### Test dataset
The ID of the test set on TIRA is `ads-in-rag-task-3-blocking-20260504-test`. Its display name is `ads-in-rag-task-3-blocking-test`.
...
To do that, select the dataset `ads-in-rag-task-3-blocking-test` from the dataset dropdown and run your submitted code.