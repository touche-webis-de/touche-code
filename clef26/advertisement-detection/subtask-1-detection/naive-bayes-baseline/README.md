# Naive Bayes Baseline for Touché'26 Advertisement Detection
...
```
./predict.py --dataset ads-in-rag-task-1-detection-spot-check-20260422-training --output predictions.jsonl --threshold 0.25
```
...
```
tira-cli code-submission --path . --task advertisement-in-retrieval-augmented-generation-2026 --dataset ads-in-rag-task-1-detection-spot-check-20260422-training --command '/predict.py --threshold 0.25'
```
...
### Test dataset
The ID of the test set on TIRA is `ads-in-rag-task-1-detection-20260504-test`. Its display name is `ads-in-rag-task-1-detection-test`.
...
To do that, select the dataset `ads-in-rag-task-1-detection-test` from the dataset dropdown and run your submitted code.
