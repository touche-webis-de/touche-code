...
- config_name: inputs
  data_files:
  - split: test
    path: ["ads-in-rag-task-1-detection-test.jsonl"]
- config_name: truths
  data_files:
  - split: test
    path: ["ads-in-rag-task-1-detection-test-labels.jsonl"]

tira_configs:
  resolve_inputs_to: "."
  resolve_truths_to: "."
  baseline:
    link: https://github.com/touche-webis-de/touche-code/tree/main/clef26/advertisement-detection/subtask-1-detection/naive-bayes-baseline
    command: /predict.py --dataset $inputDataset --threshold 0.25
    format:
      name: ["*.jsonl"]
      config:
        id_field: id
...
```
tira-cli dataset-submission --dry-run --path ads-in-rag-task-1-detection --task advertisement-in-retrieval-augmented-generation-2026 --split test
```
