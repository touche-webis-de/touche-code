...
def convert_to_dataset(train_dataframe, test_dataframe, labels):
    """
        ...
        """
    column_intersect = [x for x in (['Text'] + labels) if x in train_dataframe.columns.values]
    ...
    ds = ds.map(lambda x: {"labels": [int(x[c]) for c in ds['train'].column_names if
                                      c not in ['Text-ID', 'Sentence-ID', 'Text']]})
...
def load_tokenizer(tokenizer_dir):
    ...
    if tokenizer is None:
        ...
        print('Tokenizer was already loaded')
        sys.exit(2)
...
def predict_bert_model(dataframe, model_dir, labels):
    ...
    prediction_confidence = _multi_trainer.predict(ds['train']).predictions
    prediction = 1 * (prediction_confidence > 0.5)

    return pd.DataFrame(prediction_confidence, columns=labels)
...
def train_bert_model(train_dataframe, model_dir, labels, test_dataframe=None, num_train_epochs=20):
    """
        ...
        labels : list[str]
            The labels in the training core_data
