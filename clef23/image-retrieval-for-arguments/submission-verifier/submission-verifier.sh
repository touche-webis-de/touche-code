...
#1 dataset directory
#2 directoy with run file (.txt) to check
#3 output directory

runFile=$(echo $2/*.txt | head -n 1)
awk -F' ' 'FILENAME == ARGV[1] {
  ...
  topic = $1
  if (!(topic ~ /^[1-9][0-9]*$/ && topic > 50 && topic <= 100)) {
