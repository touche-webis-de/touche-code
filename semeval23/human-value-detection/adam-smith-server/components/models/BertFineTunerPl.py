...
from components.interface_modules.transformer_local import (load_local_model)
from components.toolbox.bert_utils import max_for_thres
...
class BertFineTunerPl(pl.LightningModule):
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        if labels is not None:
            if isinstance(self.criterion,
                          nn.BCEWithLogitsLoss):  # BCEwithLogitLOss: output => Sigmoid + BCELoss() => loss
    ...
    def validation_epoch_end(self, outputs):
        ...
        THRESHOLD_MICRO = max_for_thres(y_pred=predictions, y_true=labels, label_columns=self.label_columns,
                                        average="micro")
        THRESHOLD_MACRO = max_for_thres(y_pred=predictions, y_true=labels, label_columns=self.label_columns,
                                        average="macro")
        THRESHOLD_CUSTOM = max_for_thres(y_pred=predictions, y_true=labels, label_columns=self.label_columns,
                                         average="custom")
        ...
        for k in class_rep:
            if k == "macro avg":
                ...
                if (val_macro_precision + val_macro_recall) != 0:
                    val_custom_f1 = (
                            2 * val_macro_recall * val_macro_precision / (val_macro_recall + val_macro_precision))
                else:
                    val_custom_f1 = 0
        ...
        return {"avg_val_loss": avg_loss, "macro_avg_f1_val": val_macro_f1, "custom_f1_val": val_custom_f1,
                "roc_auc_total_micro_val": total_auroc_micro}
