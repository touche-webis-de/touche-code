...
from components.models_svm import (train_svm)

help_string = '\nUsage:  training.py [OPTIONS]' \
              '\n' \
              '\nTrain the SVM on the arguments' \
              '\n' \
              '\nOptions:' \
...
def main(argv):
    ...
    try:
        opts, args = getopt.gnu_getopt(argv, "hl:v", ["help", "levels=", "validate"])
    ...
    df_valid_all = []
    for i in range(num_levels):
        label_filepath = os.path.join(dataset_dir, 'labels-level{}.tsv'.format(levels[i]))
        if not os.path.isfile(label_filepath):
            print('The required file "labels-level{}.tsv" is not present in the core_data directory'.format(levels[i]))
            sys.exit(2)
        ...
        if validate:
            svm_f1_scores = train_svm(df_train_all[i], values[levels[i]],
                                      os.path.join(model_dir,
                                                   'svm_train_level{}_vectorizer.json'.format(levels[i])),
