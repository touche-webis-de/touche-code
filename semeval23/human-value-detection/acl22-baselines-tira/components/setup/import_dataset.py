...
def load_arguments_from_tsv(filepath, default_usage='test'):
    """
        ...
        ------
        MissingColumnError
            if the required columns "Argument ID" or "Premise" are missing in the read core_data
        IOError
...
def load_labels_from_tsv(filepath, label_order):
    """
        ...
        ------
        MissingColumnError
            if the required columns "Argument ID" or names from `label_order` are missing in the read core_data
        IOError
            if the file can't be read
        """
    try:
